...
export class R2NotifyClient extends EventEmitter<R2NotifyClientEvent> {
  ...
  private opts: Required<
    Pick<R2NotifyClientOptions, "reconnect" | "reconnectDelayMs" | "heartbeatMs" | "debug">
    ...
    const onError = (err: Event | Error) => {
      const errorObj =
        err instanceof Error
          ? err
          : new Error((err as Event)?.type ? `WebSocket error: ${(err as Event).type}` : "WebSocket error");
  ...
  // Convenience helpers...
  markAsRead() { this.emitAction("markAsRead"); }
  markAppAsRead(appId: string) { this.emitAction("markAppAsRead", { appId }); }
  markGroupAsRead(groupId: string) { this.emitAction("markGroupAsRead", { groupId }); }
  markNotificationAsRead(notificationId: string) { this.emitAction("markNotificationAsRead", { notificationId }); }

  deleteNotifications() { this.emitAction("deleteNotifications"); }
  deleteAppNotifications(appId: string) { this.emitAction("deleteAppNotifications", { appId }); }
  deleteGroupNotifications(groupId: string) { this.emitAction("deleteGroupNotifications", { groupId }); }
  deleteNotification(notificationId: string) { this.emitAction("deleteNotification", { notificationId }); }

  reloadNotifications() { this.emitAction("reloadNotifications"); }
